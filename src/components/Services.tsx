...
const servicesData = [
  ...
  },
  {
    ...
    accent: "accent",
    technicalPoints: [
        "Instalação de centros de medição conforme padrão da concessionária de energia.",
        "Adequação de instalações existentes para novas normas e demandas.",
        "Projetos para loteamentos, edifícios comerciais e residenciais."
    ]
  },
  {
    icon: Cable,
    ...
    accent: "primary",
    technicalPoints: [
        "Implantação de postes e estruturas para redes de distribuição.",
        "Instalação de transformadores e equipamentos de proteção.",
        "Manutenção preventiva e corretiva em redes aéreas."
    ]
  },
  {
    icon: FileText,
    ...
    accent: "accent",
    technicalPoints: [
        "Elaboração de projetos elétricos completos (SPDA, iluminação, etc.).",
        "Estudos de seletividade e coordenação de proteção.",
        "Projetos para correção de fator de potência e eficiência energética."
    ]
  },
  {
    icon: ClipboardCheck,
                ...
                <DialogContent className="sm:max-w-[625px] bg-card border-border">
                  <DialogHeader>
                    <DialogTitle className="flex items-center gap-4">
                      <div className={`w-12 h-12 rounded-lg bg-gradient-to-br from-${service.accent}/20 to-${service.accent}/5 flex items-center justify-center flex-shrink-0`}>
                        <Icon className={`w-6 h-6 ${getAccentColor(service.accent).split(' ')[0]}`} />
                      </div>
                      <span className="text-2xl font-heading text-foreground" translate="no">{service.title}</span>
                    </DialogTitle>
                  </DialogHeader>
                  <div className="py-4">
                    <h4 className="text-lg font-semibold text-primary mb-4">Detalhes Técnicos:</h4>
